...
const MAX_OUTPUT = Number(process.env.SCAN_COMMAND_MAX_OUTPUT || 4000);
const MAX_FILE_SCAN_DEPTH = Number(process.env.FILE_SCAN_MAX_DEPTH || 4);
const MAX_FILE_SCAN_ENTRIES = Number(process.env.FILE_SCAN_MAX_ENTRIES || 200);
const MAX_FILE_SCAN_VISITED = Number(process.env.FILE_SCAN_MAX_VISITED || 2000);
...
const CPU_FRIENDLY_QWEN_MODELS = ['qwen2.5-coder:1.5b', 'qwen2.5-coder:0.5b', 'qwen2.5-coder:3b', 'qwen2.5-coder:7b'];
...
async function collectSystemContext() {
  ...
  const checks = await Promise.all([
    ...
    command('Top memory processes', 'ps', ['-eo', 'pid,ppid,comm,%mem,%cpu,args', '--sort=-%mem', '--no-headers']),
...
<p class="muted">Collect read-only Linux context and ask <code>${OLLAMA_MODEL}</code> through Ollama at <code>${OLLAMA_HOST}</code>. CPU-friendly defaults cap context at <code>${OLLAMA_NUM_CTX}</code> tokens and responses at <code>${OLLAMA_NUM_PREDICT}</code> tokens.</p>
<div class="card">
...
<div class="card">
  <h2>File Scan</h2>
  <div class="row"><label>Path <input id="filePath" type="text" value="${defaultFilePath}"></label><label>Depth <input id="fileDepth" type="number" min="0" max="${MAX_FILE_SCAN_DEPTH}" value="1"></label><button id="fileScan">Scan Files</button></div>
...
async function findInstalledModel(requestedModel) {
  try {
    ...
    // "qwen2.5-coder" but the installed model is "qwen2.5-coder:1.5b",
    ...
    // If the configured Qwen Coder size is missing, prefer smaller installed
    // CPU-friendly tags instead of failing outright.
    if (requestedFamily === 'qwen2.5-coder') {
      const preferred = CPU_FRIENDLY_QWEN_MODELS.find(name => models.some(model => model.name === name));
...
async function generateWithModel(model, prompt, context) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Ollama returned HTTP ${response.status}: ${errorText}. Check OLLAMA_MODEL; CPU-friendly tags include qwen2.5-coder:1.5b and qwen2.5-coder:0.5b.`);
  }
